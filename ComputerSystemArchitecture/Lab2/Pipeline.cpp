...
int main()
{
    ...
    while (1) {
		...
		if(state.MEM.rd_mem==0){				//addu or sub or store 	EX-EX forwarding
			if(state.MEM.Wrt_reg_addr==state.EX.Rs){
			if (state.EX.is_I_type == 1) {//I-type
				if (state.EX.alu_op == 1) {// add
					if (state.EX.rd_mem == 1 || state.EX.wrt_mem == 1) {//Load or Store
						newState.MEM.ALUresult = state.MEM.ALUresult.to_ulong() + state.EX.Imm.to_ulong();
					}
					else
					{
						newState.MEM.ALUresult = state.MEM.ALUresult.to_ulong() + state.EX.Read_data2.to_ulong();
					}
				}
				else {//sub
					newState.MEM.ALUresult = state.MEM.ALUresult.to_ulong() - state.EX.Read_data2.to_ulong();
			...
			if (state.EX.is_I_type == 1) {//I-type
				if (state.EX.alu_op == 1) {// add
					...
					{
						newState.MEM.ALUresult = state.EX.Read_data1.to_ulong() + state.MEM.ALUresult.to_ulong();
					}
				}
				else {//sub
					newState.MEM.ALUresult = state.EX.Read_data1.to_ulong() - state.MEM.ALUresult.to_ulong();
			...
			if(newState.MEM.wrt_mem==1){
				newState.MEM.Store_data = state.MEM.ALUresult.to_ulong();
			}else{
				newState.MEM.Store_data = 0;
			}				
		...
		if(state.WB.Wrt_reg_addr==state.EX.Rs){					//MEM-EX forwarding
			if (state.EX.is_I_type == 1) {//I-type
				if (state.EX.alu_op == 1) {// add
					if (state.EX.rd_mem == 1 || state.EX.wrt_mem == 1) {//Load or Store
						newState.MEM.ALUresult = state.WB.Wrt_data.to_ulong() + state.EX.Imm.to_ulong();
					}
					else
					{
						newState.MEM.ALUresult = state.WB.Wrt_data.to_ulong() + state.EX.Read_data2.to_ulong();
					}
				}
				else {//sub
					newState.MEM.ALUresult = state.WB.Wrt_data.to_ulong() - state.EX.Read_data2.to_ulong();
		...
		if(state.WB.Wrt_reg_addr==state.EX.Rt){
			if (state.EX.is_I_type == 1) {//I-type
				if (state.EX.alu_op == 1) {// add
					...
					{
						newState.MEM.ALUresult = state.EX.Read_data1.to_ulong() + state.WB.Wrt_data.to_ulong();
					}
				}
				else {//sub
					newState.MEM.ALUresult = state.EX.Read_data1.to_ulong() - state.WB.Wrt_data.to_ulong();
